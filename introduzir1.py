...
def adaptacao():
    ...
    def turma(unidade = "", disciplina = "", disc_baixo = 0, disc_qtde = 0, turma = 0):
        ...
        time.sleep(0.2)
        pag.typewrite(str(disciplina))
        pag.press("Down", presses= disc_baixo)
        if disc_qtde > 0:
            pag.keyDown("shift")
            pag.press("down", presses= disc_qtde)
            pag.keyUp("shift")
        time.sleep(0.2)
        pag.click(sele.x, sele.y, clicks= 1)
        pag.click(conf.x, conf.y, clicks= 1)
    ...
    turma("n", "viv", disc_baixo= 2, disc_qtde= 0, turma= 86)
